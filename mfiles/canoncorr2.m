function [A,B,r,U,V,stats] = canoncorr2(X,Y,m)
...
%   variables (cols). m is a string dictating which estimator of 
%   correlation to use. Possible values are 'pearson','spearman','perbend'
%   and 'skipped'. See Wilcox 1994 and Wilcox 2004. A and B are P1-by-D and
%   P2-by-D matrices, where D = min(rank(X),rank(Y)).  The jth columns of A 
%   and B contain the canonical
...
d = min(rankX,rankY);
if strcmp(m,'pearson'), Q=Q1'*Q2;end
if strcmp(m,'spearman'), Q=corr(Q1,Q2,'type','spearman');end
if strcmp(m,'perbend')
    for i=1:size(Q1,2)
        for j=1:size(Q2,2)
            Q(i,j)=bendcorr(Q1(:,i),Q2(:,j));
        end
    end
end
if strcmp(m,'skipped')
    for i=1:size(Q1,2)
        for j=1:size(Q2,2)
            Q(i,j)=skipped_correlation(Q1(:,i),Q2(:,j));
        end
    end
end
[L,D,M] = svd(Q,0);
